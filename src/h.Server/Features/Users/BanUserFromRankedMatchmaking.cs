...
public static class BanUserFromRankedMatchmaking
{
    public class Endpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            app.MapPut("/api/v1/users/{uuid:guid}/ban", Handle)
                .AuditMethod(ctx => $"Banned user {ctx.HttpContext.GetRouteValue("uuid")}")
        ...
        [FromServices] TimeProvider timeProvider)
    {
        ...
        user.BannedFromRankedMatchmakingAt = timeProvider.GetUtcNow().UtcDateTime;
